...
- Following the release of React Hooks, it's become even clearer that `react-testing-library` is now the industry-standard for React DOM testing. Workarounds for the incompatibilities between `enzyme` are gone and we can now write leaner and more meaningful tests.
...
- **Upgrades to Babel 7, React 16.6, ESLint 5** (@julienben)
